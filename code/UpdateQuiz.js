...
export default function updateQuiz({ quiz, answer }) {
  ...
  const correctAlias = String(
    quiz.questions[i].correctAnswer.acceptedAlias
  ).toLowerCase();
  ...
  var possOptionAlias = [];
  if (!correct && hasOptions) {
    // Check if user answered an incorrect option text, also contruct possible aliases
    var ret = util.checkIncorrectOption(options, answer);
    answeredOption = ret.answeredOption;
    possOptionAlias = ret.possOptionAlias;

    if (!answeredOption) {
      // Did user answer with an alias and was the alias correct
  ...
  if (!correct && !answeredOption && hasOptions && reprompt) {
    ...
    quiz.textToSpeak =
      util.buildQuestionToSpeak(quiz.questions[quiz.index]) +
